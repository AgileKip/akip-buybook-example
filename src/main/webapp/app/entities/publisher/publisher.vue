<template>
  <div>
    <h2 id="page-heading" data-cy="PublisherHeading">
      <span v-text="$t('buyBookApp.publisher.home.title')" id="publisher-heading">Publishers</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('buyBookApp.publisher.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'PublisherCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('buyBookApp.publisher.home.createLabel')"> Create a new Publisher </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && publishers && publishers.length === 0">
      <span v-text="$t('buyBookApp.publisher.home.notFound')">No publishers found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="publishers">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('buyBookApp.publisher.name')">Name</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="buyBookApp.publisher.delete.question" data-cy="publisherDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-publisher-heading" v-text="$t('buyBookApp.publisher.delete.question', { id: removeId })">
