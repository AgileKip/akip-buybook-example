import { Component, Vue, Inject } from 'vue-property-decorator';

import TaskHandleOrderService from './task-handle-order.service';
import { TaskHandleOrderContext } from './task-handle-order.model';

const validations: any = {
  taskContext: {
    buyBookBinding: {
      buyBook: {
        purpose: {},
        purchaseReview: {},
      },
    },
  },
};

@Component({
  validations,
})
export default class TaskHandleOrderExecuteComponent extends Vue {
  private taskHandleOrderService: TaskHandleOrderService = new TaskHandleOrderService();
  private taskContext: TaskHandleOrderContext = {};
  public isSaving = false;

  beforeRouteEnter(to, from, next) {
    next(vm => {
      if (to.params.taskInstanceId) {
        vm.claimTaskInstance(to.params.taskInstanceId);
      }
    });
  }

  public claimTaskInstance(taskInstanceId) {
    this.taskHandleOrderService.claim(taskInstanceId).then(res => {
      this.taskContext = res;
    });
  }

  public previousState() {
    this.$router.go(-1);
  }

  public complete() {
    this.taskHandleOrderService.complete(this.taskContext).then(res => {
      this.$router.go(-1);
    });
  }

  public initRelationships(): void {}
}
